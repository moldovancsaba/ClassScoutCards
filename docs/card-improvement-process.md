...
The full batch-by-batch changelog (every dated entry from v1 forward, including every card-improvement
session's findings, quarantines, creates and fixes) now lives in
**`docs/card-improvement-process-changelog.md`** — split out 2026-08-10 purely for file size (the combined
file had grown to ~10,000 lines / 820KB, past the point of being practically readable or editable).
Nothing was deleted or rewritten in the split; the changelog file is a verbatim continuation of this one.
This file (the SOP proper — purpose, the loop, decision matrices, and the standing "hard-won lessons"
rules extracted from real defects) is what to read before touching anything. The changelog is where to
look for a specific prior finding, a specific card's history, or the running batch count.
